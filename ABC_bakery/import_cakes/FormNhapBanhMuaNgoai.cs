...
namespace ABC_bakery.import_cakes
{
    public partial class FormNhapBanhMuaNgoai : Form
    {
        ...
        public FormNhapBanhMuaNgoai(string role, bool permission, string taiKhoan)
        {
            InitializeComponent();
            this.role = role;
        ...
        private void connectDB()
        {
            try
            {
                SqlConnection conn = new SqlConnection(connectString);
                conn.Open();
                string loadProcedureName = "ImportInToNhapBanhMuaNgoai";
        ...
        private void setTotalPriceOnDate(string dateTime)
        {
            ...
            SqlCommand cmd1 = new SqlCommand("dbo.GetProductsByDateHaveSupplier", conn);
        ...
        private void btnChangeDate_Click_1(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(connectString);
            conn.Open();
            SqlCommand cmd = new SqlCommand("dbo.GetProductsByDateHaveSupplier", conn);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void cbSupplier_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(connectString);
            conn.Open();
            SqlCommand cmd = new SqlCommand("dbo.GetProductsBySupplier", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@name", cbSupplier.SelectedItem.ToString());
            cmd.ExecuteNonQuery();
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adapter.Fill(dt);
